class ArticlesController < ApplicationController
  ...
  def index
    if params[:query].present?
      @articles = Search::ArticleSearch.search(params[:query])
    else
  ...
  def update
    ...
    if @article.user == current_user
      ...
      # Redirect to articles path with alert if not authorized
  ...
  def destroy
    ...
    if @article.user == current_user
      ...
      respond_to do |format|
        format.html { redirect_to articles_path, alert: "You are not authorized to delete this article." }
